...
Muito bom trabalho! O grupo demonstrou que compreendeu bem os requisitos: o circuito está organizado, os sensores foram simulados adequadamente, e a lógica de irrigação foi bem justificada e implementada no código. O README é bastante funcional e o pipeline “Ir Além” (Python + R) está presente, mostrando que vocês foram além do básico. Para atingir nota máxima, poderiam aprimorar o tratamento de falhas nas leituras (ex.: checagens robustas de leitura do DHT22) e incluir no README uma seção mais detalhada de limitações e suposições (por exemplo, erros esperados, cenários que não são tratados). Grupo recebeu 1 ponto extra pela equipe com mais de três integrantes.